#!/bin/bash

if [ "$1" == "start" ]; then
	docker start iot
    docker exec -it --user $(id -u):$(id -g) iot bash
elif [ "$1" == "stop" ]; then
    docker stop iot
elif [ "$1" == "build" ]; then
    docker build -t iot_image docker/
elif [ "$1" == "create" ]; then
    if [ -z "$2" ]; then
        echo "Please provide the MySQL root password as the second argument."
        exit 1
    fi
    sed -i "17s/.*/private string connectionString = \"Server=127.0.0.1;Port=3306;Database=nodedb;Uid=root;password=$2\";/" NodeMCUAPI/Controllers/TemperatureClass.cs
    sed -i "16s/.*/private string connectionString = \"Server=127.0.0.1;Port=3306;Database=nodedb;Uid=root;password=$2\";/" NodeMCUAPI/Controllers/DeviceController.cs
    docker run -d --name iot --net=host --privileged -e MYSQL_ROOT_PASSWORD=$2 -v $(pwd):/workspace iot_image:latest
elif [ "$1" == "clean" ]; then
    docker stop iot
    docker rm iot
elif [ "$1" == "set-ip" ]; then
    if [ -z "$2" ]; then
        echo "Please provide the IP address as the second argument."
        exit 1
    fi
    sed -i "7s|.*|\"applicationUrl\": \"http://$2:46555\",|" NodeMCUAPI/Properties/launchSettings.json
    sed -i "16s|.*|\"applicationUrl\": \"http://$2:5195\",|" NodeMCUAPI/Properties/launchSettings.json
    sed -i "14s|.*|String URL = \"http://$2:5195/Temperature\";|" Node_code/Node_code.ino

else
    echo "Invalid option. Please use start, stop, build or create"
fi